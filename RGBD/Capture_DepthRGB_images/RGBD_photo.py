...
def main():
    ...
    with open("Association.txt", "w") as fout:
        while True:
            ...
            # TODO:同步检测，时间戳差异小于30ms
            # if abs(depth_timestamp - color_timestamp) < 0.03:
            if True:
                # 显示深度帧和彩色帧
                ...
                if key == 27 or key == ord("q"):  # 按下ESC或q键退出
                    ...
                    success_depth = cv2.imwrite(depth_img_name, depth_frame_obj.frame)
                    success_color = cv2.imwrite(color_img_name, color_frame_obj.frame)
                    if success_depth and success_color:
                        print(f"捕捉到第{i}组图像")
                        fout.write(
                            f"{depth_timestamp:.6f} {depth_img_name} {color_timestamp:.6f} {color_img_name}\n"
                        )
                        i += 1
                    else:
                        print(
                            f"保存图像失败，请检查{depth_img_name}和{color_img_name}是否存在"
                        )
